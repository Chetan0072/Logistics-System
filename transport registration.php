...
<div class="topnav">
  ...
  <div class="main1">
	...
	<form class="form1" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
        ...
        <input class="un " type="text" align="center" placeholder="Enter company name" name="name">
	   <input class="un " type="text" align="center" placeholder="Enter mailid" name="email">
...
$mysqli = mysqli_connect($databaseHost, $databaseUsername, $databasePassword, $databaseName);
		...
		if (isset($_POST['register'])) 
		{
			...
			$confpassword = $_POST['confpassword'];
				...
				{
					echo '<script>alert("Password And confirmpassword does not Matching")</script>'; 
				}
				else
				{
				
					// If email already exists, throw error
					$email_result = mysqli_query($mysqli, "select 'email' from transport where email='$email' and password='$password'");
					...
					else 
					{
						...
						$result   = mysqli_query($mysqli, "INSERT INTO transport(name,email,password,confpassword) VALUES('$name','$email','$password',$confpassword)");
